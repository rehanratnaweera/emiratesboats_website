...
import siteConfiguration from './.eb/make/site.json' with { type: 'json' }

// Vite config — https://vitejs.dev/config/
export default defineConfig(({ mode }) => {
  // .eb/make/deploy-preview passes `--mode development` for cached-preview builds.
  const emitSourcemaps = mode === 'development'

  return {
    base: process.env.eb_PUBLIC_URL ? `${process.env.eb_PUBLIC_URL}/` : '/',
    build: {
      ...
      rollupOptions: {
        output: {
          manualChunks: (moduleId: string) =>
            moduleId.includes('node_modules/three') ? 'three' : undefined,
        },
      },
    },
    plugins: [
      react(),
      tailwindcss(),
      ebSiteConfiguration(siteConfiguration),
      ebErrorOverlayReplay(),
      ebReactRefreshBoundaryFallback(),
      ebMakeKitPlugin({ storiesGlob: '/src/**/*.stories.{ts,tsx,js,jsx}' }),
    ],
    resolve: {
      alias: {
...
/** Applies /.eb/make/site.json to the generated document shell. */
function ebSiteConfiguration(config: ebSiteConfiguration): Plugin {
  ...
  return {
    ...
    transformIndexHtml: {
      ...
      handler(html) {
        let result = html
        result = replaceHtmlCommentSlot(result, 'eb:lang', language)
        result = replaceHtmlCommentSlot(result, 'eb:title', escapeHtmlText(title))
        result = replaceHtmlCommentSlot(result, 'eb:head-start', headStart)
        result = replaceHtmlCommentSlot(result, 'eb:head-end', headEnd)
        result = replaceHtmlCommentSlot(result, 'eb:body-start', bodyStart)
        result = replaceHtmlCommentSlot(result, 'eb:body-end', bodyEnd)
...
`,
            ...
            {
              tag: 'a',
              attrs: { class: 'eb-bypass-link', href: '#root' },
 ...
 * fixed build.
 */
function ebErrorOverlayReplay(): Plugin {
  return {
    name: 'eb-error-overlay-replay',
    apply: 'serve',
    configureServer(server) {
 ...
 */
function ebReactRefreshBoundaryFallback(): Plugin {
  const hadRefreshBoundary = new Map<string, boolean>()
  let sendFullReload: (() => void) | null = null

  return {
    name: 'eb-react-refresh-boundary-fallback',
    apply: 'serve',
    enforce: 'post',
    configureServer(server) {
...
/**
 * Serves a blank render-target page at /.eb/make/kit.html that
 * the eb preview script drives directly. The page exposes a
 * registry of every file matching `storiesGlob` on
 * window.__eb__.stories so the design surface can dynamically
 ...
 */
function ebMakeKitPlugin(options: { storiesGlob: string | string[] }): Plugin {
  const storiesGlob = Array.isArray(options.storiesGlob) ? options.storiesGlob : [options.storiesGlob]
  const ROUTE = '/.eb/make/kit.html'
  const VIRTUAL_ID = 'virtual:eb-stories'
...
<body>
<div id="eb-make-kit-root"></div>
<script type="module">
  import { stories } from 'virtual:eb-stories'
  window.__eb__ = Object.assign(window.__eb__ ?? {}, { stories })
  window.dispatchEvent(new CustomEvent('eb.ready'))
</script>
</body>
</html>`
